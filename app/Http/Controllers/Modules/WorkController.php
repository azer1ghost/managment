...
//            'paid_at_date' => $request->has('check-paid_at'),
    ...
    public function update(WorkRequest $request, Work $work): RedirectResponse
    {
        ...
        $work->parameters()->sync($parameters);
        $work->load('parameters'); // pivot dəyərləri yenilə

        if (Work::getClientServiceAmount($work) > 0) {
            // Əgər Asan imza yenicə seçilibsə və ya GB dəyişibsə
            if ($firstAsan == 1 || $request->has('parameters.' . $work::GB)) {

                // --- 1. Xidmət ID-yə görə məbləğ hesablamaları ---
                if (in_array($request->get('service_id'), [5, 6, 31, 33, 34, 35, 36, 37, 38, 7, 8, 9, 3, 4, 10, 11, 12, 49, 41, 54, 53])) {
                    $work->parameters()->updateExistingPivot($work::AMOUNT, [
                        'value' => Work::getClientServiceAmount($work) * $work->getParameter($work::SERVICECOUNT)
                    ]);
                } elseif (in_array($request->get('service_id'), [1, 16, 17, 18, 19, 20, 21, 22, 23, 26, 27, 29, 30, 42, 48])) {
                    if ($work->getRelationValue('client')->getAttribute('main_paper') > 0) {
                        if (in_array($request->get('asan_imza_id'), [22])) {
                            ...
                            $amount = (Work::getClientServiceAmount($work) * ($work->getParameter($work::GB) - $work->getParameter($work::MAINPAGE)))
                                + ($work->getRelationValue('client')->getAttribute('main_paper') * $work->getParameter($work::MAINPAGE));
                            $work->parameters()->updateExistingPivot($work::AMOUNT, ['value' => $amount]);
                        }
                    } else {
                        $amount = Work::getClientServiceAmount($work) * $work->getParameter($work::GB)
                            + $work->getRelationValue('client')->getAttribute('main_paper');
                        $work->parameters()->updateExistingPivot($work::AMOUNT, ['value' => $amount]);
                    }
                } elseif (in_array($request->get('service_id'), [2])) {
                    if ($work->getRelationValue('client')->getAttribute('qibmain_paper') > 0) {
                        if (in_array($request->get('asan_imza_id'), [22])) {
                            $work->parameters()->updateExistingPivot($work::AMOUNT, ['value' => 0]);
                        } else {
                            $amount = (Work::getClientServiceAmount($work) * ($work->getParameter($work::GB) - $work->getParameter($work::MAINPAGE)))
                                + ($work->getRelationValue('client')->getAttribute('qibmain_paper') * $work->getParameter($work::MAINPAGE));
                            $work->parameters()->updateExistingPivot($work::AMOUNT, ['value' => $amount]);
                        }
                    } else {
                        $amount = Work::getClientServiceAmount($work) * $work->getParameter($work::GB)
                            + $work->getRelationValue('client')->getAttribute('qibmain_paper');
                        $work->parameters()->updateExistingPivot($work::AMOUNT, ['value' => $amount]);
                    }
                }

                // --- 2. ƏDV (VAT) hesablamaları ---
                if (in_array($request->get('asan_imza_id'), [29, 34, 36, 39, 40, 30, 32, 33, 41, 43, 39, 46, 47, 49, 50, 48, 22, 53, 54, 55, 56, 57, 63, 80, 60, 71, 74, 83, 61, 73, 64, 72, 82, 100, 102, 98, 95, 94, 93, 91, 90, 87, 83, 78, 127, 128, 129, 130, 63, 102, 113, 117, 98, 107, 114, 73])) {
                    $work->parameters()->updateExistingPivot($work::VAT, ['value' => 0]);
                } else {
                    if (in_array($request->get('service_id'), [5, 6, 31, 33, 34, 35, 36, 37, 38, 7, 8, 9, 3, 4, 10, 11, 12, 49, 41])) {
                        $value = (Work::getClientServiceAmount($work) * $work->getParameter($work::SERVICECOUNT)) * 0.18;
                        $work->parameters()->updateExistingPivot($work::VAT, ['value' => round($value, 2)]);
                    } elseif (in_array($request->get('service_id'), [1, 16, 17, 18, 19, 20, 21, 22, 23, 26, 27, 29, 30, 42, 48])) {
                        if ($work->getRelationValue('client')->getAttribute('main_paper') > 0) {
                            $value = ((Work::getClientServiceAmount($work) * ($work->getParameter($work::GB) - $work->getParameter($work::MAINPAGE)))
                                    + ($work->getRelationValue('client')->getAttribute('main_paper') * $work->getParameter($work::MAINPAGE))) * 0.18;
                            $work->parameters()->updateExistingPivot($work::VAT, ['value' => round($value, 2)]);
                        }
                    } elseif (in_array($request->get('service_id'), [2])) {
                        if ($work->getRelationValue('client')->getAttribute('qibmain_paper') > 0) {
                            $value = ((Work::getClientServiceAmount($work) * ($work->getParameter($work::GB) - $work->getParameter($work::MAINPAGE)))
                                    + ($work->getRelationValue('client')->getAttribute('qibmain_paper') * $work->getParameter($work::MAINPAGE))) * 0.18;
                            $work->parameters()->updateExistingPivot($work::VAT, ['value' => round($value, 2)]);
                        }
                    }
                }
