...
//            'paid_at_date' => $request->has('check-paid_at'),
    ...
    public function update(WorkRequest $request, Work $work): RedirectResponse
    {
        ...
        if (!empty($client->getAttribute('phone1'))) {
                if ($request->status == 3) {
                    $message = 'Deyerli ' . $clientName . ' sizin ' . $serviceName . ' uzre isiniz tamamlandi. ' . $work->getAttribute('created_at')->format('d/m/y') . 'https://wf.qmeter.net/LvvzTj -linke kecid ederek xidmeti keyfiyyetini deyerlendirmeyinizi xahis edirik!';
