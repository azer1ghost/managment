...
//        $totalPaidAll = $totalPaidAmount + $totalPaidVat + $totalPaidIllegal;
        ...
        $calculateTotal = function ($asan_imza_ids) use ($works) {
            return $works->where('department_id', 11)
                ->whereIn('asan_imza_id', $asan_imza_ids)
                ->where('payment_method', 1)
                ->sum(function ($work) {
                    return $work->getAttribute(Work::AMOUNT) +
                        $work->getAttribute(Work::ILLEGALAMOUNT) +
                        $work->getAttribute(Work::VAT);
                });
        };
        $AMBGIASAZACash = $calculateTotal([39, 55]);
        $AMBGIMOBILCash = $calculateTotal([3, 51, 44, 38, 35, 31, 28, 26, 19, 5, 4]);
        $AMBGIGARANTCash = $calculateTotal([8, 9, 10, 25, 27, 45, 52]);
        $AMBGIRIGELCash = $calculateTotal([30, 32, 41, 43, 54]);
        $AMBGIMINDCash = $calculateTotal([29, 34, 36, 40, 46, 53]);
        $AMBGITEDORACash = $calculateTotal([48, 57]);
        $AMBGIDECLARECash = $calculateTotal([47, 49, 50, 56]);
