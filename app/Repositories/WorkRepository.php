        ...
        return Work::query()
            ...
            ->where(function($query) use ($filters, $dateRanges, $dateFilters){
                foreach ($filters as $column => $value) {
                    // 'limit' və 'statuses' filterə salınmasın
                    if (in_array($column, ['limit', 'statuses'])) continue;

                    $query->when(isset($value), function ($query) use ($column, $value, $dateRanges, $dateFilters) {
                        // Tarixlər
