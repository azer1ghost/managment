                        ...
                        <tr>
                            ...
                            <td class="d-flex">
                                @foreach(\App\Models\DailyReport::currentWeek() as $day)
                                    ...
                                    @endphp
                                    <a href="{{$route}}" class="btn mr-1 @if($day->format('Y-m-d') > now()->format('Y-m-d') || (is_null($subReport) && (auth()->user()->can('generateSubReport', App\Models\DailyReport::class) && auth()->user()->isDirector()) || auth()->user()->cannot('generateSubReport', App\Models\DailyReport::class)))) disabled @endif
