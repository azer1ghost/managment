                        ...
                        <tr>
                            ...
                            <td class="d-flex">
                                @foreach(\App\Models\DailyReport::currentWeek() as $day)
                                    ...
                                    @endphp
                                    <a href="{{$route}}" class="btn mr-2 @if($day->format('Y-m-d') > now()->format('Y-m-d')) disabled @endif
                                        @if(is_null($subReport) && $day > now()->format('Y-m-d')) btn-light
                                        @elseif(is_null($subReport)) btn-danger
                                        @elseif($day->format('Y-m-d') == now()->format('Y-m-d')) btn-primary
