...
@section('content')
    ...
    <table class="table table-responsive-sm table-hover" id="table">
        ...
        @forelse($works as $work)
            ...
            @endphp
            <tr @if($now->diffInHours($injected_at) >= 24 && $work->getAttribute('status') == $work::INJECTED) style="background: #f16b6b"
                @endif @if(is_null($work->getAttribute('user_id'))) style="background: #eed58f"
                @endif title="{{$work->getAttribute('code')}}"
                @if($work->getAttribute('painted') == 1 && $work->getAttribute('status') == $work::STARTED) style="background-color: #ff0000" @endif>
                ...
                <td style="min-width: 130px">
                    ...
                    @foreach($work->documents as $document)
                        ...
                        <a href="{{$route}}" data-toggle="tooltip" title="{{$document->name}}" target="_blank"
                           class="text-dark" style="word-break: break-word">
                            ...
                            @can('view', $work)
                                <a href="{{route('works.show', $work)}}"
                                   class="dropdown-item-text text-decoration-none">
                                    <i class="fal fa-eye pr-2 text-primary"></i>
                                </a>
                            @endcan
                                    ...
                                    <a href="{{route('works.edit', $work)}}"
                                       class="dropdown-item-text text-decoration-none">
                                        ...
                                        @elseif($work->getAttribute('user_id') == auth()->id())
                                            <i class="fal fa-arrow-right pr-2 text-success"></i>
                                        @endif
                                    </a>
                                @endcan
                            @endif
                ...
                <td>
                    ...
                    <button type="button"
                            class="docButton btn @if($work->getAttribute('doc') == 1) btn-success @else btn-danger @endif"
