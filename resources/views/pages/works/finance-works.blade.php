    ...
    <table class="table table-responsive @if($works->count()) table-responsive-md @else table-responsive-sm @endif scrollable-container"
        ...
        @if($works->isNotEmpty())
            <tr style="background: #b3b7bb" id="count">
                ...
                <td><p style="font-size: 16px" class="mb-0">Borc:<strong style="color: red">@if(($total_amount_payment + $total_vat_payment + $total_illegal_payment) == ($total_illegal_amount + $total_amount + $total_vat)) Borc Yoxdur @endif{{ (($total_illegal_amount + $total_amount + $total_vat) - ($total_amount_payment + $total_vat_payment + $total_illegal_payment)) * -1}}</strong></p></td>
                <td colspan="7"></td>
            </tr>
        @endif
        </tbody>
